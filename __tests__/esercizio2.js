...
describe("AuthTest", () => {
    xtest("Should throw error if email not provided", () => {
        expect(() => loginFn("", "password3")).toThrow("Email is required");
    });

    xtest("Should throw error if email has wrong format", () => {
        expect(() => loginFn("user3.com", "password3")).toThrow("Invalid email format");
    });

    xtest("Should throw error if email has empty spaces", () => {
